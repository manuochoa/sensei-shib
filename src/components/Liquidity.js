...
export default function Liquidity({ walletType, userAddress, setPopupShow }) {
  ...
  const quote = async (value, side) => {
    ...
    if (side === "from") {
      ...
      let exchangeRate = (value * tokenBreserve) / tokenAreserve;
      ...
      let exchangeRate = (value * tokenAreserve) / tokenBreserve;
  ...
  const getUserBalance = async () => {
    if (userAddress) {
      ...
      setInBalance(
        Moralis.Units.FromWei(inBalance, trade.tokenOut.decimals).toFixed(4)
      );
      setOutBalance(
        Moralis.Units.FromWei(outBalance, trade.tokenIn.decimals).toFixed(4)
      );
    }
  };
